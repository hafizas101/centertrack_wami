    ...
    def doCalculateHomography(self, dst_image):
        ...
        if not self.Hs:
            for srcImage in self.templates:
                H, _ = ImageProcessing.CalcHomography(srcImage, dst_image, num_of_features=500)
                Hs.append(H)
        else:
            H, _ = ImageProcessing.CalcHomography(self.templates[self.num_of_templates-1], dst_image, num_of_features=500)
